...
def makeELF(bf):
    ...
    e.extend([0x04, 0x00])                  # u16 e_shnum
    ...
    t.extend([0x2e, 0x73, 0x68, 0x73, 0x74, 0x72, 0x74, 0x62, 0x6c, 0x00]) # ".shstrtbl"
    t.extend([0x2e, 0x62, 0x73, 0x73, 0x00])      # ".bss"
    ...
    fourth_shpos = len(s)
    s.extend([0x00, 0x00, 0x00, 0x00])                          # sh_name
    s.extend([0x08, 0x00, 0x00, 0x00])                          # sh_type
    s.extend([0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])  # sh_flags
    s.extend([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])  # sh_addr
    s.extend([0x00, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])  # sh_offset
    s.extend([0x30, 0x75, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])  # sh_size
    s.extend([0x00, 0x00, 0x00, 0x00])                          # sh_link
    s.extend([0x00, 0x00, 0x00, 0x00])                          # sh_info
    s.extend([0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])  # sh_addralign
    s.extend([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])  # sh_entsize
...
def translate(source, r):
    r.extend([0xb8, 0x3c, 0x00, 0x00, 0x00])        # mov   eax, 0x3c
    r.extend([0xbf, 0x2a, 0x00, 0x00, 0x00])        # mov   edi, 0x2a
    r.extend([0x0f, 0x05])                          # syscall
